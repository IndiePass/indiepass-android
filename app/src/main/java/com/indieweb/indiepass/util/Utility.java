...
import com.indieweb.indiepass.reader.TimelineActivity;
    ...
    public static void notifyChannels(Map<String, ChannelCounter> channelCounters) {
        try {
            ...
            for (Map.Entry<String, ChannelCounter> pair : channelCounters.entrySet()) {
                ...
                //Log.d("indiepass_debug", "checking: " + id + ": " + cc.getCounter() + " / " + cc.isSource());
                for (Channel c: app.getChannelsList()) {
                    if (cc.isSource() && id.equals(c.getSourceId())) {
                        //Log.d("indiepass_debug", "counting down source " + c.getSourceId() + " with " + cc.getCounter());
                        int count = 0;
                        if (c.isCountInteger()) {
                        ...
                        //Log.d("indiepass_debug", "counting down channel " + c.getUid() + " with " + cc.getCounter() + " (" + id + ")");
                        int count = 0;
                        if (c.isCountInteger()) {
                            count = c.getUnread() + cc.getCounter();
                            if (count < 0) {
    ...
    public static String parseNetworkError(VolleyError error, Context context, int network_fail, int fail) {
        ...
        try {
            ...
            if (networkResponse != null && networkResponse.statusCode != 0 && networkResponse.data != null) {
                ...
                //Log.d("indiepass_debug", "Error: " + result);
            ...
            try {
                ...
                if (references.has(url)) {
                    ...
                    if (ref.has("content")) {
                        ...
                        if (content.has("text")) {
                            if (level == 1 && item.getReference().length() > 0) {
                                //Log.d("indiepass_debug", "swap on level 1: " + item.getReference());
                                item.setSwapReference(false);
                                item.setTextContent(item.getReference());
                            }
                            //Log.d("indiepass_debug", "content: " + content.getString("text"));
                    ...
                    if (checkRecursive && ref.has("quotation-of")) {
                        //Log.d("indiepass_debug", "going recursive");
