...
import com.indieweb.indiepass.model.Cache;
import com.indieweb.indiepass.model.ChannelCounter;
import com.indieweb.indiepass.model.TimelineItem;
import com.indieweb.indiepass.model.TimelineStyle;
...
public class TimelineActivity extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private void markItemsReadWhileScrolling() {
        try {
            ...
            //Log.d("indiepass_debug", "First visible: " + position + " - last visible: " + listView.getLastVisiblePosition());
            if (listView.getLastVisiblePosition() == (adapter.getCount() - 1)) {
                position = listView.getLastVisiblePosition();
                //Log.d("indiepass_debug", "Count: " + adapter.getCount() + " - Last position: " + listView.getLastVisiblePosition());
            }
            //Log.d("indiepass_debug", "position: " + position);
            ...
            while (itemPosition <= position) {
                ...
                //Log.d("indiepass_debug", "Position:" + itemPosition + " - " + item.getName());
                if (item != null && !item.isRead()) {
                    //Log.d("indiepass_debug", "Marking as read: " + item.getName());
                    ...
                    if (channelCounters.containsKey(item.getChannelId())) {
                        ...
                        //Log.d("indiepass_debug", "updated channel: " + item.getChannelId() + " - " + cc.getCounter());
                    }
                    else {
                        ChannelCounter cc = new ChannelCounter();
                        channelCounters.put(item.getChannelId(), cc);
                        //Log.d("indiepass_debug", "added channel: " + item.getChannelId());
                    }
                    ...
                    if (item.getSourceId().length() > 0 && Preferences.getPreference(TimelineActivity.this, "channel_show_sources", false)) {
                        if (channelCounters.containsKey(item.getSourceId())) {
                            ...
                            //Log.d("indiepass_debug", "updated source: " + item.getSourceId() + " - " + cc.getCounter());
                        }
                        else {
                            ChannelCounter cc = new ChannelCounter();
                            cc.setSource(true);
                            channelCounters.put(item.getSourceId(), cc);
                            //Log.d("indiepass_debug", "added source: " + item.getSourceId() + " - " + cc.getCounter());
                        }
                    }
                }
                itemPosition++;
            ...
            //Log.d("indiepass_debug", "Exception marking read: " + e.getMessage());
        }
    }
}
